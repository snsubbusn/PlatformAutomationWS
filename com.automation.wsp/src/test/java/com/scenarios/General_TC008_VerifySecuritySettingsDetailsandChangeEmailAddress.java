...
public class General_TC008_VerifySecuritySettingsDetailsandChangeEmailAddress extends Action_Method
{

	public void preferenceAndSecurity() throws Exception
	{
		
		logger = extent.startTest("General_TC008_VerifySecuritySettingsDetailsandChangeEmailAddress");
		driver.manage().timeouts().implicitlyWait(30,TimeUnit.SECONDS);
		logger.assignAuthor("Natasha");
		logger.assignCategory("General");
		logger.setDescription("Login with any WS user and click on ‘Settings’. Verify a new tab is opened and navigate to the new tab. Navigate to Security Settings and click on Change Email Address. Enter the details and verify the save button is enabled. Log out and close the browser");
		...
		String b = "false";
		if(sp.VerifyChangeEmailAddress().equals(b))
		{
			logger.log(LogStatus.INFO,"Entered new email id and OTP to verify if Save button is enabled");
