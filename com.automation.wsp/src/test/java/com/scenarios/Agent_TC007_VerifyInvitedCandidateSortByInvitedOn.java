...
public class Agent_TC007_VerifyInvitedCandidateSortByInvitedOn extends Action_Method{
	...
	public void verifySortByInvitedOnofInvitedCandidatePageofAgencyPartner() throws InterruptedException {
		...
		if(count.contains("exception")) {
			...
			logger.log(LogStatus.INFO, "Verify the Sort by \"Invited On\" of the Invited Candidate page of Agency Partner");
			...
			logger.log(LogStatus.INFO, "Sort the candidates by \"Invited On\" and verify the sort value displayed");

			String aft = ic.sortByInvitedOn();
			if(aft.contains("Invited")) {
				logger.log(LogStatus.PASS, "The candidates are sorted by \"Invited On\" successfully");
			}else {
				logger.log(LogStatus.FAIL, "Failed to sort the candidates by \"Invited On\"");
			}
		}
